...
Rscript := /usr/local/bin/Rscript --no-restore --no-save
R := R_LIBS=Rlibrary3.5.2 /usr/local/bin/R CMD BATCH --no-restore --no-save
...
.PHONY: precheck all prepare_data cluster summarise figs memoryusage mergeparameters

## ------------------------------------------------------------------------------------ ##
## Define rules
## ------------------------------------------------------------------------------------ ##
## Default rule
all: precheck precheck_confirm prepare_data prepare_data_confirm cluster summarise figs memoryusage mergeparameters parameterrange

precheck:
	@echo "" ## Print empty line
	@echo "DATASETS: ${DATASETS}"
	@echo "METHODS: ${METHODS}"
	@echo "FILTERINGS: ${FILTERINGS}"
	@echo "PCTKEEP: ${PCTKEEP}"
	@echo "DATASET_FOR_SCALABILITY: ${DATASET_FOR_SCALABILITY}"
	@echo "DATASET_FOR_PARAM_RANGES: ${DATASET_FOR_PARAM_RANGES}"
	@echo "METHOD_FOR_PARAM_RANGES: ${METHOD_FOR_PARAM_RANGES}"
	@echo "" ## Print empty line
...
parameterrange: output/investigate_parameter_range/investigate_parameter_range_$(METHOD_FOR_PARAM_RANGE).rds
...
plots/manuscript/figure4.rds plots/manuscript/figure5.rds plots/performance/seurat_diagnostics.rds \
...
output/scalability/scalability_$(1).rds: data/sce_filteredExpr10/sce_filteredExpr10_$(DATASET_FOR_SCALABILITY).rds \
Rscripts/evaluate_results/calculate_scalability.R Rscripts/clustering/apply_$(1).R \
parameter_settings/sce_filteredExpr10_$(DATASET_FOR_SCALABILITY).json parameter_settings/sce_filteredExpr10_$(DATASET_FOR_SCALABILITY)_$(1).json \
...
## Investigate parameter ranges
## ------------------------------------------------------------------------------------ ##
output/investigate_parameter_range/investigate_parameter_range_$(METHOD_FOR_PARAM_RANGE).rds: data/sce_filteredExpr10/sce_filteredExpr10_$(DATASET_FOR_PARAM_RANGES).rds \
Rscripts/investigate_parameter_range/investigate_parameter_range_$(METHOD_FOR_PARAM_RANGE).R Rscripts/clustering/apply_RtsneKmeans.R
	mkdir -p $(@D)
	$(R) "--args scefile='$<' k=8 outrds='$@'" Rscripts/investigate_parameter_range/investigate_parameter_range_$(METHOD_FOR_PARAM_RANGE).R Rout/investigate_parameter_range_$(METHOD_FOR_PARAM_RANGE).Rout
...
# Uncomment the lines below for Seurat-specific analysis.
